...
class AudioInterface(Node):
    def __init__(self):
        ...
        self.AUDIO_FOLDER = self.get_parameter("folder").value

        self.audio_pub = self.create_publisher(CompressedImage, 'audio_bytes', 10)
        self.track_list = [f for f in os.listdir(self.AUDIO_FOLDER) if f.endswith('.mp3') or f.endswith('.wav')]
    ...
    def audio_loop(self):
        while True:
            ...
            if not playing or not track_list:
                ...
                continue
            with self.lock:
                track = os.path.join(self.AUDIO_FOLDER, self.track_list[self.current_track_index])
    ...
    def change_volume(self, delta):
        with self.lock:
            self.volume = max(0.0, min(8.0, self.volume + delta))
            if self.playing:
                self.restart = True
