...
// Задача "простоя" для ядра 1
void idleTask(void *pvParameters) {
  while (1) {
    idleCount++;
    // Можно добавить задержку 0, чтобы не мешать системе
    taskYIELD();
  }
}
...
uint8_t getCPULoad() {
  ...
  uint32_t now = millis();
  uint32_t idle = idleCount;
  uint32_t dt = now - lastTime;
  uint32_t didle = idle - lastIdle;
  lastIdle = idle;
  lastTime = now;
  if (dt == 0) return 0;
  // Эмпирически калибруйте значение ниже для вашей частоты:
  float idlePercent = (float)didle / (float)dt / 100.0f;
  if (idlePercent > 1.0) idlePercent = 1.0;
  if (idlePercent < 0.0) idlePercent = 0.0;
  return (uint8_t)((1.0f - idlePercent) * 100.0f);
}

void drawGraph() {
  tft.fillRect(GRAPH_X, GRAPH_Y, GRAPH_WIDTH, GRAPH_HEIGHT, ST77XX_BLACK);
  for (int x = 1; x < GRAPH_WIDTH; x++) {
    int y1 = GRAPH_Y + GRAPH_HEIGHT - cpuLoadHistory[x - 1];
    int y2 = GRAPH_Y + GRAPH_HEIGHT - cpuLoadHistory[x];
    tft.drawLine(GRAPH_X + x - 1, y1, GRAPH_X + x, y2, ST77XX_GREEN);
  }
